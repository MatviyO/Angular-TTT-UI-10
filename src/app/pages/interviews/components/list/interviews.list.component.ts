...
@Component({
  ...
  providers: [InterviewsListConfig, CountryStatesService, CompanyResourceService, StatesService, InterviewOutcomeService, InterviewStatusHelper],
})
export class InterviewsListComponent extends BaseSortableListWithTriggersDirective<InterviewRecord> implements OnInit {
  ...
  constructor(
    ...
    @Inject(InterviewOutcomeService) interviewOutcomeSvc: IResourceService<InterviewOutcome>,
  ) {
    super(config);
    this.interviewOutcomeSvc = interviewOutcomeSvc;
  }
  ...
  getStatusProcess(stages: InterviewStageV2[]): string {
    ...
    if (_outccome) {
      ...
      let interviews = stages.filter(x => x.recordType === 'InterviewStage');
      if (interviews && interviews.length > 0 ) {
        interviews = interviews.sort((b, a) => a.index - b.index).sort((b, a) => Date.parse((a.date as string).split('/').reverse().join('-')) - Date.parse((b.date as string).split('/').reverse().join('-')));
        return `waiting for outcome, last interview stage ${this.formatDate(new Date(interviews[0].date))}`;
      } else {
