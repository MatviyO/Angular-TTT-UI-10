...
import {
  ...
  MonthsService,
  ProfileResourceService, SettingService,
  WorkforceTrainingCompanyResourceService
} from '../../../../../core/data';
import {
  EmploymentRecord, Profile,
  WorkforceTrainingCompany,
  WorkforceTrainingPersonal,
  WorkforceTrainingPersonalItem
} from '../../../../../core/model';
...
@Component({
  ...
  templateUrl: './workforce-personal.details.component.html',
...
export class WorkforcePersonalDetailsComponent extends DetailsWithTriggersDirective<WorkforceTrainingPersonal> implements DoCheck {
  ...
  ngDoCheck(): any {
    from(this.entity.items)
      .subscribe(x => (x.reimbursementAmount = x.totalHours * x.wage / 2).toFixed(2),
      );
  }

  dataLoaded(data: WorkforceTrainingPersonal): any {
    if (!data) {
      ...
      this.employmentCompanySvc.query(`id==${this.entity.employmentRecordId}`)
        .then((result: EmploymentRecord[]) => {
          if (result.length > 0) {
            ...
            this.profileSvc
              .query(`id=${result[0]
                .applicationId}`, '', [{key: 'apt', value: 'CanHillerHousingAllowance'}, {key: 'apt', value: 'HillerEmploymentId'}])
              .then(res => {
                if (res.length > 0) {
      ...
      if (data.employmentRecord && data.employmentRecord.application) {
        this.navigation
          .addNavigation(`${data.employmentRecord.application.firstName} ${data.employmentRecord.application.lastName}`,
