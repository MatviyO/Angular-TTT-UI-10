...
export class ObservableService {
    ...
    observableSourceCompanies = (keyword: any): Observable<any[]> => {
        if (keyword) {
            return this.observableSourceAny(`(name.contains("${this.replaceSpecialCharacters(keyword)}"))`, '/api/Companies?where=isProspect=false and isActive=true and');
        } else {
            return of([]);
        }
    }
    ...
    observableSourceTypeProfile = ( forType: string, filter: string, keyword: any): Observable<any[]> => {
        ...
        // filterStr += `(${fName}) and isActive=true ${filter ? `and (${filter}) ` : ''} `;
